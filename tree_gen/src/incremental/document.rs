// src/incremental/document.rs
use ropey::Rope;
use cstree::{
    green::GreenNode,
    syntax::{SyntaxNode, ResolvedElementRef},
    text::{TextRange,TextSize},
};

use crate::{
    engine::SyntaxEngine,
    language::Syntax as LangSyntax,
};

/// Manages one open document: text buffer, parse‐state, and incremental updates.
pub struct DocumentSession<Sy>
where
    Sy: LangSyntax + 'static,
{
    /// File identifier (e.g. `url.to_string()`).
    pub uri: String,

    /// O(log n) editable text buffer.
    buffer: Rope,

    /// Core Chumsky↔cstree driver (owns its own cache).
    engine: SyntaxEngine<'static, 'static, Sy>,

    /// Current red‐tree root.
    root: SyntaxNode<Sy>,
}

impl<'src,Sy> DocumentSession<Sy>
where
    Sy: LangSyntax + 'static,
{
    /// One‐shot full parse on open.
    pub fn open<Err>(uri: String, initial: &str) -> Result<Self, Err>
    where
        Err: chumsky::error::Error<'src, &'src str> + 'src,
    {
        let mut engine = SyntaxEngine::default();
        let green = engine.parse_full::<Err>(initial)?;
