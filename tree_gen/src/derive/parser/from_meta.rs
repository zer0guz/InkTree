...
pub trait FromMeta: Sized {
    ...
    fn from_list(list: &MetaList, _: Option<&Ident>) -> Result<Self, ElementError> {
        Err(syn::Error::new_spanned(
            list,
            "from_meta list not supported for this thing todo",
        ))
        .context(MetaSnafu)?
    }
    ...
    fn from_path(path: &Path, _: Option<&Ident>) -> Result<Self, ElementError> {
        Err(syn::Error::new_spanned(
            path,
            "from_meta path not supported for this thing todo",
        ))
        .context(MetaSnafu)?
    }
    fn from_name_value(
        ...
        Err(syn::Error::new_spanned(
            name_value,
            "from_meta name_value not supported for this thing todo",
        ))
        .context(MetaSnafu)?
    }
}
