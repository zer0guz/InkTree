...
pub struct AstShape {
    pub kind: AstShapeKind,
    pub name: Ident,
}

impl AstShape {
    pub fn new(kind: AstShapeKind, name: Ident) -> Self {
        Self { kind, name }
    }
}
...
impl DslExpr {
    pub fn ast_shape(
        ...
        let kind = match self {
            DslExpr::Just(ident) => AstShapeKind::Node {
                fields: vec![Field {
                    name: ident.clone(),
                    kind: ident.clone(),
            ...
            DslExpr::Call { name, .. } => AstShapeKind::Node {
                fields: vec![Field {
                    name: name.clone(),
                    kind: name.clone(),
                    cardinality: Cardinality::One,
            ...
            DslExpr::Alt(exprs) => {
                ...
                AstShapeKind::Enum { variants }
            }
        };

        AstShape::new(kind, rule_name.clone())
    }
}
    ...
    pub fn codegen(&self, lang_ident: &Ident, name: &Ident) -> TokenStream {
        ...
        match &self.kind {
            ...
            AstShapeKind::Pratt {
                ...
                let atom_ast = format_ident!("{}Ast", atom);
