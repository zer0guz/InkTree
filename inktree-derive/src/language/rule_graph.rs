...
impl RuleGraph {
    ...
    pub(crate) fn into_recursive_info(
        &mut self,
        idents: &HashMap<Ident, &Element>,
    ) -> RecursionInfo {
        let (components, node_to_comp) = compute_sccs(&self.adj);
        let left_recursive = self.detect_left_recursion(idents);
    ...
    fn compute_nullable(rules: &HashMap<Ident, &Element>) -> HashMap<Ident, bool> {
    ...
    fn build_left_corner_graph(
        elements: &HashMap<Ident, &Element>,
        nullable: &HashMap<Ident, bool>,
    ) -> HashMap<Ident, HashSet<Ident>> {
        let mut lc: HashMap<Ident, HashSet<Ident>> = elements
            .keys()
            .cloned()
            .map(|k| (k, HashSet::new()))
            .collect();

        for (a, element) in elements {
    ...
    fn detect_left_recursion(&self, rules: &HashMap<Ident, &Element>) -> HashSet<Ident> {
